 ...
 * Uses OpenAI API for natural language to SQL conversion and metadata enrichment.
 */
@Slf4j
@Service
@ConditionalOnProperty(name = "oneapi.llm.provider", havingValue = "openai")
public class OpenAILLMService implements LLMService {

    @Value("${oneapi.llm.openai.api-key}")
    private String apiKey;

    @Value("${oneapi.llm.openai.model:gpt-4o-mini}")
    private String model;

    @Value("${oneapi.llm.openai.temperature:0.1}")
    ...
    public NaturalLanguageQueryResponse convertToSQL(
        ...
        log.info("Converting natural language to SQL using OpenAI: {}", request.getQuestion());

        try {
            String prompt = buildPrompt(request.getQuestion(), schema, request.getContext());
            String generatedSQL = callOpenAIAPI(prompt);
            ...
            throw new RuntimeException("Failed to generate SQL using OpenAI: " + e.getMessage(), e);
        }
    }
    ...
    public String explainSQL(String sql, DatabaseSchemaContext schema) {
        try {
            ...
            log.error("Error explaining SQL with OpenAI", e);
    ...
    public SQLValidationResult validateSQL(String sql, DatabaseSchemaContext schema) {
        // Basic validation - could be enhanced with OpenAI API
    ...
    private String callOpenAIAPI(String prompt) {
        ...
        systemMessage.put("content", "You are a helpful assistant that generates SQL queries and enriches database metadata.");
    ...
    public String callLLM(String prompt) {
        log.debug("Calling OpenAI API with prompt of length: {}", prompt.length());
