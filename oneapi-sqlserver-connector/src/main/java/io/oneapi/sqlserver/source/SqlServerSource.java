...
/**
 * Microsoft SQL Server implementation of Source.
 */
public class SqlServerSource extends AbstractDatabaseSource<JDBCType, JdbcDatabase> {
    ...
    protected Set<String> getExcludedInternalNameSpaces() {
        return Set.of("information_schema", "sys", "INFORMATION_SCHEMA");
    }
    ...
    public JsonNode toDatabaseConfig(JsonNode config) {
        ...
        int port = config.has(JdbcUtils.PORT_KEY) ? config.get(JdbcUtils.PORT_KEY).asInt() : 1433;
        ...
        // SQL Server JDBC URL format: jdbc:sqlserver://host:port;databaseName=database;encrypt=true;trustServerCertificate=true
        String jdbcUrl = String.format("jdbc:sqlserver://%s:%d;databaseName=%s;encrypt=true;trustServerCertificate=true",
                host, port, database);
    ...
    private List<Field<JDBCType>> discoverTableFields(JdbcDatabase database, String schema, String table) throws SQLException {
        ...
        database.query(query, List.of(schema, table), rs -> {
            while (rs.next()) {
                ...
                JDBCType jdbcType = mapSqlServerTypeToJdbcType(dataType);
    ...
    private JDBCType mapSqlServerTypeToJdbcType(String sqlServerType) {
        return switch (sqlServerType.toLowerCase()) {
            ...
            case "varchar", "nvarchar", "text", "ntext" -> JDBCType.VARCHAR;
            ...
            case "binary", "varbinary", "image" -> JDBCType.BINARY;
    ...
    protected Map<String, List<String>> discoverPrimaryKeys(JdbcDatabase database, List<TableInfo<Field<JDBCType>>> tableInfos) {
        ...
        for (TableInfo<Field<JDBCType>> tableInfo : tableInfos) {
            String schema = tableInfo.getNameSpace();
            String table = tableInfo.getName();
            String fullyQualifiedName = schema + "." + table;
